//! # gateway-guard
//!
//! Pre/post/stream hook stages with block/mask/flag/log actions, observe-only and dry-run modes; WASM (wasmtime) sandbox so a slow plugin can't breach the latency SLA or crash the server.
//!
...
pub const CRATE: &str = "gateway-guard";
